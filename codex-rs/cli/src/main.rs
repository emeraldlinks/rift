...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Provider(mut provider_cli)) => {
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
                "provider",
            ...
            prepend_config_flags(
                &mut provider_cli.config_overrides,
                root_config_overrides.clone(),
            );
            let loader_overrides =
                loader_overrides_for_profile(interactive.config_profile_v2.as_ref())?;
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        | Subcommand::Fork(_)
        | Subcommand::Mcp(_)
        | Subcommand::Provider(_)
        | Subcommand::Sandbox(_)
        | Subcommand::Debug(DebugCommand {
            subcommand: DebugSubcommand::PromptInput(_),
        }) => Ok(Some(profile_v2)),
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `codex mcp`: `codex`, `codex exec`, `codex review`, `codex resume`, `codex queue`, `codex archive`, `codex delete`, `codex unarchive`, `codex fork`, `codex mcp`, `codex provider`, `codex sandbox`, and `codex debug prompt-input`."
        ),
    }
}

...
pub fn validate_reserved_model_provider_ids(
    ...
    // Bedrock providers allow partial overrides (base_url, auth, aws settings)
    // so they are excluded from the "cannot be overridden" check.
    let mut conflicts = model_providers
        ...
        .filter(|key| {
            ...
            ) && BUILT_IN_PROVIDER_IDS.contains(&key.as_str())
        })
        .map(|key| format!("`{key}`"))
        .collect::<Vec<_>>();
